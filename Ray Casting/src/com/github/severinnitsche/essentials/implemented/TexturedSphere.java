...
public class TexturedSphere extends Sphere {
  ...
  public ObjectInformation collide(Ray ray) {
    ...
    double tu = normal.getSpherical(1)/MathUtil.TAU+.5;//Math.atan2(normal.get(0),normal.get(2)) / MathUtil.TAU + .5;//normal.get(0)/2+.5;//Math.asin(normal.get(0))/ MathUtil.PI+.5;
    double tv = normal.getSpherical(0)/MathUtil.PI;//normal.get(1) * .5 + .5;//normal.get(1)/2+.5;//Math.asin(normal.get(1))/ MathUtil.PI+.5;
